...
import { Activity, TrendingUp, TrendingDown, Zap, Bell, BellOff, Volume2, VolumeX, Wallet, Target, BarChart3, PieChart, Play, Square, RefreshCw } from "lucide-react";
import { api, HealthStatus, RSIResponse, TickerResponse, Candle, StrategyResult, BacktestResult, BotStatus } from '@/lib/api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setLastUpdated(new Date().toLocaleString('tr-TR', { timeZone: 'Europe/Istanbul' }));
    }

    // --- MERGED FRONTEND EFFECTS ---
  ...
  // --- MERGED FRONTEND HANDLERS ---
  const handleStartBot = async () => {
    try {
      await api.startBot(symbol);
      const status = await api.getBotStatus();
      setBotStatus(status);
      sendNotification('Bot Started', `Trading bot started for ${symbol}`);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleStopBot = async () => {
    try {
      ...
      const status = await api.getBotStatus();
      setBotStatus(status);
      sendNotification('Bot Stopped', 'Trading bot has been stopped');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleRunStrategy = async () => {
    try {
        ...
        if (result?.result?.signal !== 'NEUTRAL') {
             playAdaptiveSound(result.result.signal, 85, 'MANUAL_RUN');
        }
    } catch (err) {
        console.error(err);
    } finally {
  ...
  useEffect(() => {
    ...
    const pollBackend = async () => {
        try {
            ...
            // Map backend data to Signal interface
            signalsData.forEach((data: any) => {
                 let createdAtVal = null;
                 // Backend likely returns ISO string for datetime
                 if (data.created_at) {
                    const dateObj = new Date(data.created_at);
                     createdAtVal = { seconds: Math.floor(dateObj.getTime() / 1000) };
                 }

                 const signalItem = { ...data, created_at: createdAtVal };
            ...
            Object.entries(symbolStats).forEach(([symbol, stats]) => {
                if (stats.buys > maxBuys) { maxBuys = stats.buys; bestSymbol = symbol.replace("/USDT", ""); }
                if (stats.sells > maxSells) { maxSells = stats.sells; worstSymbol = symbol.replace("/USDT", ""); }
            });

            setSignals(tradeSignals.slice(0, 20));
            setSystemLogs(logs.slice(0, 5));
            setStats({ active_signals: tradeSignals.length, buy_signals: buy, sell_signals: sell });
            ...
            // PROCESS TRADES
             const tradesList: TradeRecord[] = tradesData.map((t: any) => ({
                ...t,
                id: t.id,
                created_at: t.created_at ? { seconds: Math.floor(new Date(t.created_at).getTime() / 1000) } : null
             }));
    ...
    return () => clearInterval(interval);
  }, [lastSignalCount, notificationsEnabled, soundEnabled, symbol]); // Add symbol dependency
      ...
      <div className="relative p-4 md:p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* CHART */}
            <Card className="lg:col-span-2 bg-slate-900/50 border-slate-700/50 backdrop-blur-xl">
               <CardHeader className="flex flex-row items-center justify-between pb-2">
                 ...
                 <div className="flex gap-2">
                    {/* Symbol Selector generic */}
                    {['BTC', 'ETH', 'SOL', 'AVAX'].map(s => (
                        <button 
                            key={s}
                            onClick={() => setSymbol(s)} 
                            className={`px-3 py-1 rounded text-xs font-bold transition-all ${symbol === s ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                        >
                            {s}
                        </button>
                    ))}
            ...
            <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-xl">
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 text-center">
                        <div className="text-sm text-slate-400 mb-2">Bot Durumu</div>
                        {botStatus?.is_running ? (
                            <div className="flex flex-col items-center gap-2">
                                <Badge className="bg-green-500/20 text-green-400 hover:bg-green-500/30 text-lg px-6 py-2 animate-pulse">
                                    AKTİF
                                </Badge>
                                <p className="text-xs text-slate-500">Otomatik alım-satım açık</p>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center gap-2">
                                <Badge variant="outline" className="text-slate-400 border-slate-600 text-lg px-6 py-2">
                                    DURDURULDU
                                </Badge>
                                <p className="text-xs text-slate-500">Otomatik işlem kapalı</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                         <button
                            ...
                            className={`p-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
                                botStatus?.is_running 
                    ...
                    <div className="space-y-3">
                        <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Manuel Strateji</label>
                        <select 
                            value={selectedStrategy}
                            onChange={(e) => setSelectedStrategy(e.target.value)}
                            className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                        >
                            <option value="sma_crossover">SMA Crossover (Fast/Slow)</option>
                            <option value="supertrend">SuperTrend (BUM)</option>
                            <option value="rsi">RSI Reversal</option>
                            <option value="macd">MACD Momentum</option>
                            <option value="proteus">Proteus AI (ML)</option>
                        </select>
                        <button 
                            onClick={handleRunStrategy}
                            disabled={loading}
                            className="w-full bg-blue-600 hover:bg-blue-500 text-white p-3 rounded-lg font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                        >
