...
export const api = {
  ...
  getSignals: async (limit: number = 20): Promise<any[]> => {
    return (await axios.get(`${API_URL}/api/v1/signals`, { params: { limit } })).data;
  },

  getTrades: async (limit: number = 20): Promise<any[]> => {
    return (await axios.get(`${API_URL}/api/v1/trades`, { params: { limit } })).data;
  },

  getPortfolio: async (): Promise<any> => {
    return (await axios.get(`${API_URL}/api/v1/portfolio`)).data;
  }
};
